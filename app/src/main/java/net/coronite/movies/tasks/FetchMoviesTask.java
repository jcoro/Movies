...
public class FetchMoviesTask extends AsyncTask<String, Void, ArrayList<MovieItem>> {
    ...
    private AsyncTaskCompleteListener<ArrayList<MovieItem>> mListener;

    public FetchMoviesTask(AsyncTaskCompleteListener<ArrayList<MovieItem>> listener) {
        mListener = listener;
    }
    ...
    private ArrayList<MovieItem> getMovieDataFromJson(String movieJsonStr)
        ...
        ArrayList<MovieItem> movieResults = new ArrayList<>();

        for(int i = 0; i < resultsArray.length(); i++) {

            // Get the JSON object representing the movie
            JSONObject singleMovieJson = resultsArray.getJSONObject(i);
            MovieItem singleMovie = new MovieItem(singleMovieJson);
            movieResults.add(singleMovie);
    ...
    protected void onPostExecute(ArrayList<MovieItem> result) {
        // sends the result to FetchMoviesTaskCompleteListener.onTaskComplete located in the MovieFragment.
